...
# Generate full bind9 configuration from a (greyspace) hosts file
# (glsomlo@cert.org, June 2015)
#
# We assume a well-behaved hosts file consisting of valid lines
# of the form <ip_addr fqdn> (a file auto-generated by the greyspace
...
SRC_MHOSTS='/var/lib/greyspace/etc/hosts.vmail'

# input delegations file (hashes to be sourced into our namespace):
SRC_DELEG='/etc/greyspace/delegations.dns'

# output hosts file (<ip_addr fqdn> for all virtual DNS servers impersonated):
NAMED_HOSTS='/var/lib/greyspace/etc/hosts.named'

# output named.conf
NAMED_CONF='/var/lib/greyspace/etc/named.conf'

# output folder for zone files:
NAMED_ZD='/var/lib/greyspace/named'
...
The optional command line arguments are:
    ...
    -w <web_hosts>   name of the input hosts list containing <ip_addr fqdn>
                     pairs of all virtual Web greyspace hosts to be hosted by
                     the web server;
                     (default: $SRC_WHOSTS).
    ...
    -m <mail_hosts>  name of the input hosts list containing <ip_addr fqdn>
                     pairs of all of greyspace's virtual mail servers;
                     (default: $SRC_MHOSTS).
    ...
    -n <dns_hosts>   name of the output hosts list containing <ip_addr fqdn>
                     pairs for all virtual DNS greyspace servers supported by
...
Required next steps may include:
    ...
    - (re-)start greyspace-named service
"

[ "$QUIET_GEN" == "no" ] && echo "$SUCCESS_BLURB"


# FIXME: Sort out SELinux policy associated with greyspace package !!!
